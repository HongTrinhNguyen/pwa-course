...
test('PRODUCT_001 - Tạo product thành công tren dev', async ({ page, browserName, isMobile }) => {
    console.log(`Running on browser: ${browserName}`);
    test.skip(isMobile,'Skip if running on Mobile')
    process.env.ENV = 'dev';
    const env = process.env.ENV || 'dev';
    const data = env === 'dev' ? dataDev : dataProd;
    const username = loadEnvInfo(env).validUsername || '';
...
test('PRODUCT_001 - Tạo product thành công tren prod', async ({ page, browserName, isMobile }) => {
    console.log(`Running on browser: ${browserName}`);
    test.skip(isMobile,'Skip if running on Mobile')
